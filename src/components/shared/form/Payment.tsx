...
const Payment = () => {
   ...
   const [selectedPyament, setselectedPyament] = useState('');
   ...
   const handleBankChange = (event: any) => {
      ...
      const selectedBank = config.BANKS.find((bank) => bank.name === event);
   ...
   return (
      <>
         <div className='flex-center'>
            {config.PAYMENT.map((item, i) => (
               <Button key={i} className={`${item.type === selectedPyament ? 'bg-slate-400' : ''}`} onClick={() => handlePaymentType(item.type)}>
                  <Image src={item.icon} className='w-12' width={400} height={400} alt={item.type} />
               </Button>
            ))}
         </div>
            ...
            <div className='card secondary mt-20 w-full max-w-xl p-4 sm:p-5'>
               ...
               <form>
                  <div className='mt-7 space-y-4'>
                     ...
                     <div className='flex flex-col items-center gap-4 justify-center space-x-2 pt-4'>
                        <SubmitButton>
                           Comfirmar
                        </SubmitButton>
                        <SubmitButton className='before:bg-red-600 text-red-600 border-red-600'>
                           Cancelar
                        </SubmitButton>
                     </div>
                  </div>
